...
public class ResourceUtils {
    ...
    public static boolean copyFileFromAssets(final String assetsFilePath, final String destFilePath) {
        boolean res = true;
        try {
            String[] assets = BaseQiShuiApplication.getContext().getAssets().list(assetsFilePath);
            if (assets.length > 0) {
                for (String asset : assets) {
                    res &= copyFileFromAssets(assetsFilePath + "/" + asset, destFilePath + "/" + asset);
                }
            } else {
                res = writeFileFromIS(destFilePath, BaseQiShuiApplication.getContext().getAssets().open(assetsFilePath), false);
            }
        } catch (IOException e) {
            e.printStackTrace();
            res = false;
    ...
    public static String readAssets2String(final String assetsFilePath, final String charsetName) {
        InputStream is;
        try {
            is = BaseQiShuiApplication.getContext().getAssets().open(assetsFilePath);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static List<String> readAssets2List(final String assetsPath,
        ...
        try {
            return is2List(BaseQiShuiApplication.getContext().getResources().getAssets().open(assetsPath), charsetName);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static boolean copyFileFromRaw(@RawRes final int resId, final String destFilePath) {
        return writeFileFromIS(
                destFilePath,
                BaseQiShuiApplication.getContext().getResources().openRawResource(resId),
                false
        );
    }
    ...
    public static String readRaw2String(@RawRes final int resId, final String charsetName) {
        InputStream is = BaseQiShuiApplication.getContext().getResources().openRawResource(resId);
        byte[] bytes = is2Bytes(is);
        if (bytes == null) return null;
        if (isSpace(charsetName)) {
            return new String(bytes);
    ...
    public static List<String> readRaw2List(@RawRes final int resId, final String charsetName) {
        return is2List(BaseQiShuiApplication.getContext().getResources().openRawResource(resId), charsetName);
    }
